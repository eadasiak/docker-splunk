  ...
  if [[ $__configured == "false" ]]; then
    ...
    for n in {1..10}; do
      if [[ -n $(eval echo \$\{SPLUNK_FORWARD_SERVER_${n}\}) ]]; then
        if [[ -n $(eval echo \$\{SPLUNK_FORWARD_SERVER_${n}_CERT\}) ]]; then
          printenv SPLUNK_FORWARD_SERVER_${n}_CERT > /opt/splunk/etc/SPLUNK_FORWARD_SERVER_${n}_CERT
        fi
        if [[ -n $(eval echo \$\{SPLUNK_FORWARD_SERVER_${n}_CA\}) ]]; then
          printenv SPLUNK_FORWARD_SERVER_${n}_CA > /opt/splunk/etc/SPLUNK_FORWARD_SERVER_${n}_CA
        fi
    ...
    # Setup local HTTP Event Collector
    if [[ -n ${SPLUNK_HEC_USESSL} ]]; then
      sudo -HEu ${SPLUNK_USER} sh -c "${SPLUNK_HOME}/bin/splunk http-event-collector update -name http -uri https://localhost:8089 -disabled 0 -enable-ssl $(eval echo \$\{SPLUNK_HEC_USESSL\})"
    fi
    if [[ -n ${SPLUNK_HEC_USEJSON} ]]; then
      sudo -HEu ${SPLUNK_USER} sh -c "${SPLUNK_HOME}/bin/splunk http-event-collector update -name http -uri https://localhost:8089 -disabled 0 -output-format json"
    fi
    for n in {1..10}; do
      if [[ -n $(eval echo \${SPLUNK_HEC_${n}\}) ]]; then
        sudo -HEu ${SPLUNK_USER} sh -c "${SPLUNK_HOME}/bin/splunk http-event-collector create new-token -name $(eval echo \$\{SPLUNK_HEC_${n}\}) -uri https://localhost:8089 -disabled 0 -outputgroup $(eval echo \$\{SPLUNK_HEC_${n}_OUTPUTGROUP\})"
